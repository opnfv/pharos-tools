...
class SearchableSelectMultipleWidget {
    constructor(format_vars, field_dataset, field_initial) {
        ...
        if( this.format_vars.show_from_noentry )
        {
            this.search("");
        }
    }
    ...
    search_field_init() {
        ...
        {
            //this.select_item(String(this.initial[i]));
            this.select_item(String(elem));
        }
    ...
    add_to_tree(str, id, trie) // does this work? TODO: check if this actually does anything
    {
        let inner_trie = trie;
        while( str )
        {
    ...
    search(input)
    {
        if( input.length == 0 && !this.format_vars.show_from_noentry){
            this.dropdown([]);
            return;
        }
        else if( input.length == 0 && this.format_vars.show_from_noentry)
        {
    ...
    serialize(trie)
    {
        ...
        */
        ...
        for( const key in trie )
        {
            if(key.length > 1)
            {
                continue; // is there a point to this? TODO
            }
    ...
    collate(trees)
    {
        ...
        */
        ...
        for( const tree of trees )
        {
            ...
            //for( let j=0; j<available_IDs.length; j++){
            for( const available_ID of available_IDs ) {
                const itemID = available_ID;
    ...
    dropdown(ids)
    {
        ...
        */
        ...
        for( const id in ids )
        {
            ...
            result_button.setAttribute('onclick', 'searchable_select_multiple_widget.select_item("' + obj['id'] + '")');
    ...
    {
        //TODO make faster
        const item = this.items[item_id]['id'];
        if( (this.format_vars.selectable_limit > -1 && this.added_items.length < this.format_vars.selectable_limit) || this.format_vars.selectable_limit < 0 )
        {
            if( this.added_items.indexOf(item) == -1 )
            {
    ...
    {
        const item = Object.values(this.items)[item_ref];
        const index = this.added_items.indexOf(item);
    ...
    {
        document.getElementById("added_number").innerText = this.added_items.length;
        ...
        for( const item_id of this.added_items ) // TODO: convert to let..of
        {
            ...
            list_html += '<div class="list_entry">'
                ...
                + Object.values(this.items).indexOf(item)
