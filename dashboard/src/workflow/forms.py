...
class SearchableSelectMultipleField(forms.Field):
    def __init__(self, *args, required=True, widget=None, label=None, disabled=False,
        ...
        super().__init__(disabled=disabled, **kwargs)  # kwargs may need to be manually unpacked here

    def clean(self, data):
        print("data before cleaning: " + str(data) + ", " + str(type(data)))
        data = data[0]
        print("data after cleaning: " + str(data) + ", " + str(type(data)))
        if not data:
            print("no items were selected")
            raise ValidationError("Nothing was selected")
        data_as_list = json.loads(data)
        print("data_as_list: " + str(data_as_list))
        ...
        print("returning items: " + str(items))
        return items
...
class SearchableSelectGenericForm(forms.Form):
    ...
    def generate_items(self, queryset):
        raise Exception("SearchableSelectGenericForm does not implement concrete generate_items()")
...
class ResourceSelectorForm(SearchableSelectGenericForm):
    def generate_items(self, queryset):
        items = {}
