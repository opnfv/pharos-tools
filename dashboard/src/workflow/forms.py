...
class ResourceSelectorForm(forms.Form):
    ...
    def __init__(self, data=None, **kwargs):
        ...
        attrs = self.build_search_widget_attrs(chosen_resource, bundle, edit)
        queryset = GenericResourceBundle.objects.all()
        if data and 'user' in data:
            user = data.get('user', None)
            user_bundles = GenericResourceBundle.objects.filter(owner=user)
            public_bundles = GenericResourceBundle.objects.filter(public=True)
            queryset = user_bundles | public_bundles

        self.fields['generic_resource_bundle'] = forms.CharField(
                widget=SearchableSelectMultipleWidget(attrs=attrs)
                )

    def build_search_widget_attrs(self, chosen_resource, bundle, edit):
        resources = {}
        for res in GenericResourceBundle.objects.select_related("owner").all():
            displayable = {}
...
class HardwareDefinitionForm(forms.Form):
    ...
    #filter_field = MultipleSelectFilterField()
