...
class Define_Hardware(WorkflowStep):
    ...
    def get_context(self):
        ...
        context['form'] = form
        self.log.debug("Define Hardware Context: %s", json.dumps(context, indent=2))
        return context
    ...
    def update_models(self, data):
        ...
        self.log.debug("Define_Hardware.update_models with data: %s", json.dumps(data, indent=2))
        models = self.repo_get(self.repo.GRESOURCE_BUNDLE_MODELS, {})
        ...
        # return to repo
        self.log.debug("Adding to repo: %s", json.dumps(models, indent=2))
...
class Define_Nets(WorkflowStep):
    ...
    def get_context(self):
        ...
        self.log.debug("Final context: %s", json.dumps(context, indent=2))
        return context
    ...
    def parseXml(self, xmlString):
        ...
        self.log.debug("hosts: %s", json.dumps(hosts, indent=2))
        self.log.debug("interfaces: %s", json.dumps(interfaces, indent=2))
...
class Resource_Meta_Info(WorkflowStep):
    ...
    def get_context(self):
        ...
        self.log.debug("Resource_Meta_Info context: %s", json.dumps(context, indent=2))
        return context
    ...
    def post_render(self, request):
        ...
        if form.is_valid():
            ...
            self.repo_put(self.repo.CONFIRMATION, confirm)
            self.metastep.set_valid("Step Completed")
            self.log.debug("Resource_Meta_Info post_render models: %s", json.dumps(models, indent=2))
        else:
