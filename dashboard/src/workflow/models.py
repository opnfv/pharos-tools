...
class Repository():
                ...
                for interface_set in models['interfaces'].values():
                    for interface in interface_set:
                        try:
                            ...
                            print("saving interface " + interface.profile.name + " on host " + interface.host.resource.name)
                            interface.save()
                        except Exception as e:
                ...
                for resource_name, mapping in models['vlans'].items():
                    for profile_name, vlan_set in mapping.items():
                        ...
                        for vlan in vlan_set:
                            try:
                                print("adding vlan " + vlan.vlan_id +" to interface " + interface.profile.name)
                                vlan.save()
                                interface.vlans.add(vlan)
                            except Exception as e:
                                return "GRB, saving vlan " + str(vlan) + " failed. CODE:0x0017"
            else:
    ...
    def make_software_config_bundle(self):
        ...
        if 'opnfv' in models:
            ...
            pass
            #return "SWC, no opnfv config in models. CODE:0x000a"
