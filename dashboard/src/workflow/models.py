    ...
    def post_render(self, request):
        ...
        if form.is_valid():
            ...
            if data == "True":
                ...
                print("data was true so empty response:")
                return HttpResponse('')
            elif data == "False":
                context["bypassed"] = "true"
                messages.add_message(request, messages.SUCCESS, "Canceled")
                print("data was false so yeah")
                return render(request, self.template, context)
            else:
                pass
...
class Repository():
    ...
    #If one doesn't exist, pick from list, or do a creation which pulls you out of the current workflow
    RESULT_GRB = "grb generated by resource workflow"
    RESULT_SWC = "swc generated by config workflow"
    RESULT_IMG = "img generated by image workflow"
    RESULT_BOOK = "booking generated by booking workflow"
