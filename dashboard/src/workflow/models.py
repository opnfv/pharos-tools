...
class Repository():
    ...
    def make_opnfv_config(self):
        ...
        for host_role in host_roles:
            config = config_bundle.hostConfigurations.get(host__resource__name=host_role['host_name'])
            HostOPNFVConfig.objects.create(role=host_role['role'], config=config)
