...
class SessionManager():
    ...
    def pop_workflow(self):
        if(len(self.workflows) <= 1):  # current workflow is only remaining one, should still populate RESULT
            ...
            return (False, current_repo.el[current_repo.HAS_RESULT], current_repo.el[current_repo.RESULT])
        ...
        self.workflows.pop()
        current_repo = self.workflows[-1].repository
        return (True, current_repo.el[current_repo.HAS_RESULT], current_repo.el[current_repo.RESULT])
