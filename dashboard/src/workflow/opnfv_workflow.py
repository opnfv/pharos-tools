...
class Assign_Network_Roles(WorkflowStep):
    ...
    def get_context(self):
        ...
        context['formid'] = "network_role_form"
    ...
    def post_render(self, request):
        ...
        Formset, kwargs = self.create_netformset(roles, config_bundle)
        #kwargs.pop("initial")
        net_role_formset = Formset(request.POST, **kwargs)
...
class Assign_Host_Roles(WorkflowStep):  # taken verbatim from Define_Software in sw workflow, merge the two?
    ...
    def get_context(self):
        ...
        Formset, kwargs = self.create_hostformset(hostlist=config.bundle.getHosts())
        context["formset"] = Formset(**kwargs)
    ...
    def post_render(self, request):
        HostFormset, kwargs = self.create_hostformset()
        formset = HostFormset(request.POST, **kwargs)
        ...
        if formset.is_valid():
            for form in formset:
                ...
                role = form.cleaned_data['role']
                mapping = get_host_role_mapping(host_roles, hostname)
                mapping['role'] = role
