...
class QuickBookingValidFormTestCase(TestCase):
    ...
    def test_with_invalid_installer(self):
        response = self.client.post('/booking/quick/', {
            ...
            'hostname': 'hostnamefieldcontentstring',
            'image': str(self.image.id),
            'installer': str(self.installer.id + 100),
            'scenario': str(self.scenario.id),
        })
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(Booking.objects.first())
    ...
    def test_with_invalid_scenario(self):
        response = self.client.post('/booking/quick/', {
            ...
            'image': str(self.image.id),
            'installer': str(self.installer.id),
            'scenario': str(self.scenario.id + 100),
        })
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(Booking.objects.first())
    ...
    def test_with_invalid_host_id(self):
        response = self.client.post('/booking/quick/', {
            'filter_field': '{"hosts":[{"host_' + str(self.host_profile.id + 100) + '":"true"}], "labs": [{"lab_' + str(self.lab.lab_user.id) + '":"true"}]}',
            'lab_' + str(self.lab.lab_user.id) + '_selected': 'true',
            'host_' + str(self.host_profile.id + 100) + '_selected': 'true',
            'purpose': 'purposefieldcontentstring',
            'project': 'projectfieldcontentstring',
            'length': '3',
            'ignore_this': 1,
    ...
    def test_with_invalid_lab_id(self):
        response = self.client.post('/booking/quick/', {
            'filter_field': '{"hosts":[{"host_' + str(self.host_profile.id) + '":"true"}], "labs": [{"lab_' + str(self.lab.lab_user.id + 100) + '":"true"}]}',
            'lab_' + str(self.lab.lab_user.id + 100) + '_selected': 'true',
            'host_' + str(self.host_profile.id) + '_selected': 'true',
            'purpose': 'purposefieldcontentstring',
            'project': 'projectfieldcontentstring',
            'length': '3',
    ...
    def test_with_garbage_users_field(self):  # behavior is undefined in this case currently, we should clarify
        response = self.client.post('/booking/quick/', {
            'filter_field': '{"hosts":[{"host_' + str(self.host_profile.id) + '":"true"}], "labs": [{"lab_' + str(self.lab.lab_user.id) + '":"true"}]}',
            'lab_' + str(self.lab.lab_user.id) + '_selected': 'true',
            'host_' + str(self.host_profile.id) + '_selected': 'true',
            ...
            'length': '3',
            'ignore_this': 1,
            'users': 'X�]QP�槰DP�+m���h�U�_�yJA:.rDi��QN|.��C��n�P��F!��D�����5ȅj�9�LV��',  # output from /dev/urandom
            'hostname': 'hostnamefieldcontentstring',
            'image': str(self.image.id),
            'installer': str(self.installer.id),
            'scenario': str(self.scenario.id),
