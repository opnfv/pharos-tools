...
from workflow.forms import SearchableSelectMultipleWidget, MultipleSelectFilterField, MultipleSelectFilterWidget, FormUtils
from account.models import UserProfile
from resource_inventory.models import Image, Installer, Scenario, OPNFVRole, ConfigBundle
...
class QuickBookingForm(forms.Form):
    ...
    def __init__(self, data=None, *args, user=None, **kwargs):
        ...
        images = None
        if user:
            images = forms.ModelChoiceField(queryset=Image.objects.filter(Q(public=True) | Q(owner=user)), required=False)
        else:    
            images = forms.ModelChoiceField(queryset=Image.objects.all(), required=False)
