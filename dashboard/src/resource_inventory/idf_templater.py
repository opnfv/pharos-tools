...
class IDFTemplater:
    ...
    def get_public_net(self, booking):
        config = OPNFVConfig.objects.get(bundle=booking.config)
        public_role = config.networks.get(name="public")
        public_vlan = Vlan.objects.filter(network=public_role.network).first()
        public_network = PublicNetwork.objects.get(vlan=public_vlan.vlan, lab=booking.lab)
        self.networks['public']['vlan'] = public_vlan.vlan
        public = {}
    ...
    def get_oob_net(self, booking):
        net = {}
        hosts = booking.bundle.hosts.all()
    ...
    def get_single_net_config(self, booking, net_name):
        ...
        self.networks[net_name]['vlan'] = vlan.vlan
        net = {}
    ...
    def get_fuel_nodes(self, booking):
        nodes = booking.resource.hosts.exclude(config__opnfvRole__name="jumphost")
        for host in nodes:
            node = {}
