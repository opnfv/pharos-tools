...
class Job(models.Model):
    ...
    """
    #hardware = models.ManyToManyField(HardwareConfig, through='HostHardwareConfig')
    #network = models.ManyToManyField(NetworkConfig, through='HostNetworkConfig')
    #software = models.ForeignKey(SoftwareConfig, on_delete=models.DO_NOTHING)
    #access = models.ManyToManyField(AccessConfig)
    #relations now accessible through job.{softwarelinks, accesslinks, hardwarelinks, networklinks}
    #as querydicts
    #with corresponding configs accessible through job.<QueryDict: relation>.config
    ...
    """
        d = {}
        hardware_configs = self.hardware.all()
        if hardware_configs.exists():
            d['hardware'] = {}
            for config in hardware_configs:
                hostname = config.relationship.first().host.labid
                d['hardware'][hostname] = config.to_dict()

        network_configs = self.network.all()
        if network_configs.exists():
            d['network'] = {}
            for config in network_configs:
                hostname = config.relationship.first().host.labid
                d['network'][hostname] = config.to_dict()

        if self.software:
            d['software'] = self.software.to_dict()
    ...
    def get_delta(self, status):
        ...
        for relation in HostNetworkRelation.objects.filter(job=self).filter(status=status):
            if not 'network' in d:
                d['network'] = {}
                print("creating network key")
            d['network'][relation.task_id] = relation.config.get_delta()

        j['payload'] = d
        #if not self.delta:
        #    self.delta = self.to_json()
        #    print("giving job self delta: " + str(self.delta))
        #    self.save()
        #return json.loads(self.delta)
        #filtered_relations = TaskConfig.objects.filter(job=self).filter(status=status)
...
class AccessRelation(TaskRelation):
    ...
    #job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name='accesslinks')
...
class SoftwareRelation(TaskRelation):
    #status = models.CharField(max_length=20)
    #job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name='softwarelinks')
...
class HostHardwareRelation(TaskRelation):
    #status = models.CharField(max_length=20)
    #job = models.ForeignKey(Job, on_delete=models.CASCADE, related_name='hardwarelinks')
...
class HostNetworkRelation(TaskRelation):
    #status = models.CharField(max_length=20)
    #job = models.ForeignKey(Job, on_delete=models.DO_NOTHING, related_name='networklinks')
    host = models.ForeignKey(Host, on_delete=models.CASCADE)
